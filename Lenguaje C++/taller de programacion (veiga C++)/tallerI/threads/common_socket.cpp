...
/*
La clase Socket la tome del ejemplo dado por la catedra, lei que se podia hacer
pero hacia falta tambien esta aclaracion para que sepan de donde la tome
*/
...
int Socket::recibir(void *data, unsigned int sz, bool *was_closed) {
    ...
    *was_closed = false;
    ...
    while (received < sz) {
        ...
        if (s <= 0) {
        	if (s < 0){
        		std::string error = "Error al recibir los bytes";
...
int Socket::enviar(const void *data, unsigned int sz) {
    unsigned int sent = 0;
    ...
    while (sent < sz) {
        ...
        if (s <= 0) {
            if (s < 0){
            	std::string error = "Error al enviar los bytes";
        		throw SocketError(error);
        	}
            // Si el socket fue cerrado (s == 0) o hubo un error (s < 0)
...
Socket Socket::accept() {
    ...
    //NOLINTNEXTLINE
    if(skt == -1){
    	std::string error = "Error al conectar el socket";
        throw SocketError(error);
	}
